...
class PatchServicesNTPSettingsEndpointRequest(BaseModel):
    """
    ...
    serverauthkey: StrictStr = Field(description="The NTP server authentication key.<br><br>This field is only available when the following conditions are met:<br>- `serverauth` must be equal to `true`<br>")
