...
class PatchServicesHAProxyFrontendActionEndpointRequest(BaseModel):
    """
    ...
    acl: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1024)]] = Field(default=None, description="The name of the frontend ACL this action is associated with.<br>")
    backend: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1024)]] = Field(default=None, description="The backend to use when an ACL match is found.<br><br>This field is only available when the following conditions are met:<br>- `action` must be equal to `'use_backend'`<br>")
    ...
    __properties: ClassVar[List[str]] = ["action", "acl", "backend", "customaction", "deny_status", "realm", "rule", "lua_function", "name", "fmt", "find", "replace", "path", "status", "reason", "parent_id", "id"]
