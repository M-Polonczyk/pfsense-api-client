...
class PatchServicesDNSResolverSettingsEndpointRequest(BaseModel):
    """
    ...
    sslcertref: StrictStr = Field(description="The SSL/TLS certificate to use for the DNS Resolver service.<br><br>This field is only available when the following conditions are met:<br>- `enablessl` must be equal to `true`<br>")
